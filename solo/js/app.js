...
(() => {
  ...
  const $ = (id) => document.getElementById(id),
    STORAGE_KEY = "enduro_solo_v01",
    DAY_KEY = "enduro_solo_v01_day_history";
  const DEFAULT_NAMES = ["Esteban"];
  const PALETTE = [
    "#2a4cff",
    "#00c2ff",
    "#00d084",
  ...
  document.addEventListener("keydown", (evento) => {
    ...
    if (!piloto) {
      showToast("No hay un piloto activo");
      return;
    }
    if (!running || launchMode) {
      showToast("Boton recibido: primero inicia el cronometro");
      return;
    }

    mark(piloto.id, 0);
    showToast(`Enduro Solo: ${piloto.name} registrado`);
  });
  [
    elPrepSec,
    elGap,
  ...
  // ENDURO LINK V0.6 LAB
  // El Nodo 1 corresponde al primer piloto de la lista (Esteban).
  // La primera lectura solo establece una referencia para no registrar
  // accidentalmente el ultimo evento viejo al abrir o recargar la pagina.
  let eventoLinkInicializado = false;
  let ultimaClaveEventoLink = "";

  async function consultarEnduroLink() {
    try {
      const respuesta = await fetch("/api/estado", { cache: "no-store" });
      if (!respuesta.ok) return;
      const dato = await respuesta.json();
      if (!dato.hayEvento) return;

      const clave = `${dato.nodo}:${dato.evento}:${dato.actualizadoBaseMs}`;
      if (!eventoLinkInicializado) {
        eventoLinkInicializado = true;
        ultimaClaveEventoLink = clave;
        return;
      }
      if (clave === ultimaClaveEventoLink) return;
      ultimaClaveEventoLink = clave;

      const indiceLista = Number(dato.nodo) - 1;
      const piloto = riders[indiceLista];
      if (!piloto || piloto.active === false) {
        showToast(`Nodo ${dato.nodo}: piloto no disponible`);
        return;
      }
      if (!running || launchMode) {
        showToast(`Nodo ${dato.nodo} recibido: cronometro detenido`);
        return;
      }

      const pilotosActivos = getActiveRiders();
      const indiceActivo = pilotosActivos.findIndex((r) => r.id === piloto.id);
      if (indiceActivo < 0) return;

      mark(piloto.id, indiceActivo);
      showToast(`Enduro Link: ${piloto.name} registrado`);
    } catch (_) {
      // La interfaz manual continua funcionando aunque la base no responda.
    }
  }

  // En modo Solo no se consulta una Base ESP-NOW: el boton llega
  // directamente al navegador como una tecla Bluetooth.
